...
//アイテム一覧の取得
$get = getGraphList($authKey,$hostname);

foreach ( $get as  $temp ){
	if(empty($temp))
		break;
	$i=0;
	foreach ( $temp->items as $itemdata ){

		if($i==0){
		
			$itemstr = $itemdata->name;
		}else{
			$itemstr = $itemstr.",".$itemdata->name;
		}
		error_log($i);
		$i++;
	}
...
//ホスト名からホストデータ
function getGraphList ($auth, $hostname )
{
	...
	$json    = array("jsonrpc" => "2.0", "method" => "graph.get","auth" => $auth, "params" => array("output" => "extend","selectItems" => "extend","filter"=>array("host" =>"$hostname"),"sortfield" => "name","sortorder" => "ASC" ), "id" => 1);
	$ch = curl_init($apiUrl);
