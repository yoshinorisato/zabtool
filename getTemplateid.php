<?php
function getTemplateid( $templName, $auth )
{
	//$templName = 'Template_sysUptime';
	include "config.php";
	global $errorMsg;
	$errorMsg = "";
	...
	$json    = array("jsonrpc" => "2.0", "method" => "template.get","auth" => $auth, "params" => array("output" => "extend", "filter" => array("host" => $templName ) ), "id" => 3);
	$ch = curl_init($apiUrl);
	...
	elseif(isset($ret->result))
	{
		...
		//templateidにテンプレートIDが取得される
		//echo "TEMPLATE ".@$ret->result[0]->templateid." END";
