...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(CPU使用率)".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名にCPU使用率が含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/CPU使用率/", $triggername)){
					//CPU使用率ではないとき
					continue;
				}
