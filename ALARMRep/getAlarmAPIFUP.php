...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(AP無線IF Up)".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	//ヘッダの生成
	$csvout = "";
	$csvout = $csvout. "【 AP無線IF Up 】の異常検出数\n";
	$csvout = $csvout. "(単位：件)\n";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名にAP無線IF Upが含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/AP無線IF Up/u", $triggername)){
					//AP無線IF Upではないとき
					continue;
				}
