...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(帯域幅使用率(受信))".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	//ヘッダの生成
	$csvout = "";
	$csvout = $csvout. "【 帯域幅使用率(受信) 】の異常検出数\n";
	$csvout = $csvout. "(単位：件)\n";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名に帯域幅使用率(受信)が含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/帯域幅使用率(受信)/u", $triggername)){
					//帯域幅使用率(受信)ではないとき
					continue;
				}
