...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(ストーム発生)".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名にストーム発生が含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/ストーム発生/", $triggername)){
					//ストーム発生ではないとき
					continue;
				}
