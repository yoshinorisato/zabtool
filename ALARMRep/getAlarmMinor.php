...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(MNランプ状態)".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名にMNランプ状態が含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/MNランプ状態/", $triggername)){
					//MNランプ状態ではないとき
					continue;
				}
