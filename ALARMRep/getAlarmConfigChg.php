...
global $authKey,$errorMsg;
	...
	//CSVファイルに出力
	$outputFile = mb_convert_encoding("アラーム統計(コンフィグ変更発生)".$minYear.$minMonth.$minDay."～".$maxYear.$maxMonth.$maxDay ,"SJIS","UTF-8").".csv";
	...
	//ヘッダの生成
	$csvout = "";
	$csvout = $csvout. "【 コンフィグ変更発生 】の異常検出数\n";
	$csvout = $csvout. "(単位：件)\n";
	...
	foreach( $retArray->result as $groupdata){
		...
		foreach( $retList->result as $eventData){	
			...
			if(isset($eventData->relatedObject->description)){
				// トリガー名にコンフィグ変更発生が含まれていればカウント
				$triggername = $eventData->relatedObject->description;
				if(!preg_match("/コンフィグ変更発生/", $triggername)){
					//コンフィグ変更発生ではないとき
					continue;
				}
